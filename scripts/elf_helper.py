...
class ElfHelper:
    ...
    def find_kobjects(self, syms):
        ...
        for die in variables:
            ...
            if ((addr < kram_start or addr >= kram_end)
                ...
                self.debug_die(die, "object '%s' found in invalid location %s" %
                          (name, hex(addr)))
                continue
            ...
            self.debug("symbol '%s' at %s contains %d object(s)" % (name, hex(addr),
                                                           len(objs)))
    ...
    def write_gperf_table(self, fp, header, footer, objs, static_begin, static_end):
        fp.write(header)
        ...
        for obj_addr, ko in objs.items():
            ...
            byte_str = struct.pack("<I" if self.little_endian else ">I", obj_addr)
            fp.write("\"")
